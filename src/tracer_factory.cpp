...
namespace newrelic {
    ...
    opentracing::expected<std::shared_ptr<opentracing::Tracer>> TracerFactory::MakeTracer(const char *configuration, std::string &error_message) const noexcept {
        ...
        auto socket = Config::getDaemonSocket();
        bool newrelicInit = newrelic_init(socket, 0);
        Log::debug("({}) TracerFactory::MakeTracer newrelicInit: {} daemonSocket: {}", (const void *) this, newrelicInit, socket);
